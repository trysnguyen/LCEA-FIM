...
def hill_climbing(solution, Eval, budget, all_nodes, max_iterations=50):
    """
    Args:
        solution: giải pháp hiện tại (danh sách node ID)
        Eval: hàm đánh giá fitness
        budget: số lượng node cần chọn
        all_nodes: tất cả các node trong graph
        max_iterations: số lần lặp tối đa

    Returns:
        giải pháp đã được cải thiện (local optimum)
    """
    current_solution = copy.deepcopy(solution)
    current_fitness = Eval(current_solution)
    ...
    while iteration < max_iterations:
        ...
        candidate_nodes = [n for n in all_nodes if n not in current_solution]

        for i in range(len(current_solution)):
            original_node = current_solution[i]

            for candidate in candidate_nodes:
                # Tạo neighbor bằng cách thay thế node tại vị trí i
                current_solution[i] = candidate
                neighbor_fitness = Eval(current_solution)
                ...
                current_solution[i] = original_node

        if found_better:
            current_solution = best_neighbor
            current_fitness = best_neighbor_fitness
        else:
            # Không tìm được neighbor nào tốt hơn, đã đạt local optimum
            break
...
for graphname in graphnames:
    print(graphname)
    for budget in [40]:
        g = pickle.load(open('networks/twitter.pickle'.format(graphname), 'rb'))
        ng = list(g.nodes())
        ngIndex = {}
        ...
        for attr_idx, attribute in enumerate(attributes):
            ...
            for run in range(num_runs):
                ...
                address = 'networks/twitter.txt'.format(graphname)
                ...
                # Hill Climbing phase - cải thiện giải pháp sau EA
                print("Áp dụng Hill Climbing để cải thiện giải pháp...")
                SS = hill_climbing(SS, Eval, budget, list(partition.keys()), max_iterations=50)
                print("Hill Climbing hoàn thành")
